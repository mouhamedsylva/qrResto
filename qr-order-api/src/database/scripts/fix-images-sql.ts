...
/**
 * Script SQL simple pour corriger les URLs Unsplash obsolètes
 * Usage: npm run fix:images-sql
 */

// Mapping des URLs Unsplash valides par mot-clé
...
function findBestImageUrl(itemName: string, categoryName: string): string {
  ...
  return DEFAULT_IMAGES[categoryLower] || DEFAULT_IMAGES['default'];
}
...
async function fixImages() {
  ...
  try {
    ...
    // Récupérer tous les articles avec des URLs obsolètes ou vides
    const [rows] = await connection.execute(`
      ...
      WHERE mi.imageUrl LIKE '%source.unsplash.com%'
         OR mi.imageUrl IS NULL
         OR mi.imageUrl = ''
    `);

    const items = rows as any[];
    console.log(`📊 ${items.length} articles à corriger\n`);
    ...
    for (const item of items) {
      const newImageUrl = findBestImageUrl(item.name, item.categoryName || 'default');

      await connection.execute(
        'UPDATE menu_items SET imageUrl = ? WHERE id = ?',
        [newImageUrl, item.id]
      );

      console.log(`✅ ${item.name}`);
      console.log(`   Catégorie: ${item.categoryName || 'Non définie'}`);
      console.log(`   Avant: ${item.imageUrl || '(vide)'}`);
