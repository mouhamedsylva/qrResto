...
export class AddReservationsTable1776259760658 implements MigrationInterface {
    name = 'AddReservationsTable1776259760658'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`reservations\` (\`id\` varchar(36) NOT NULL, \`customerName\` varchar(255) NOT NULL, \`customerPhone\` varchar(255) NOT NULL, \`customerEmail\` varchar(255) NULL, \`partySize\` int NOT NULL, \`reservationDate\` date NOT NULL, \`reservationTime\` time NOT NULL, \`status\` enum ('PENDING', 'CONFIRMED', 'CANCELLED', 'COMPLETED', 'NO_SHOW') NOT NULL DEFAULT 'PENDING', \`notes\` text NULL, \`reminderSent\` tinyint NOT NULL DEFAULT 0, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`restaurantId\` varchar(36) NULL, \`tableId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
